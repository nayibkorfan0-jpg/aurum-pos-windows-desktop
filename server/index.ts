...
// Use userData directory for session storage in desktop mode
const sessionDir = process.env.SERVER_USER_DATA_DIR 
  ? path.join(process.env.SERVER_USER_DATA_DIR, 'data')
  : './data';
...
// Session configuration - optimized for both Replit and desktop environments
app.use(session({
  ...
  cookie: {
    secure: process.env.RUNTIME !== 'desktop', // HTTPS only for Replit, HTTP OK for desktop
