...
// Create storage instance based on runtime environment
const runtime = (process.env.RUNTIME || 'web').toLowerCase();

let storageInstance: IStorage;

if (runtime === 'desktop') {
  console.log('🖥️  Using SQLite storage for desktop mode');
  storageInstance = new SQLiteStorage();
} else if (process.env.DATABASE_URL) {
  console.log('☁️  Using PostgreSQL storage for web mode');
  storageInstance = new PostgresStorage();
} else {
  console.log('🖥️  No DATABASE_URL found, defaulting to SQLite storage');
  storageInstance = new SQLiteStorage();
}

// Initialize admin user on startup
storageInstance.initializeAdminUser().catch((error) => {
