...
// Ensure SQLite storage in desktop mode
process.env.RUNTIME = 'desktop';
...
async function findAvailablePort(startPort: number = 5000, endPort: number = 5010): Promise<number> {
...
async function startExpressServer(): Promise<void> {
  return new Promise(async (resolve, reject) => {
    ...
    if (isDev) {
      ...
      try {
        SERVER_PORT = await findAvailablePort(5000, 5010);
    ...
    const serverPath = path.join(process.env.DIST_ELECTRON || __dirname, '../dist/index.js');
    ...
    serverProcess = spawn(process.execPath, [serverPath], {
      env: {
        ...
        SESSION_SECRET: 'aurum-pos-desktop-session-secret-2024-production-key-32chars',
        RUNTIME: 'desktop',
        SERVER_USER_DATA_DIR: userDataPath,
    ...
    serverProcess.stdout?.on('data', (data) => {
      const logData = `${new Date().toISOString()} [STDOUT]: ${data}`;
      console.log(logData);
      logStream.write(logData);
    });

    serverProcess.stderr?.on('data', (data) => {
      const logData = `${new Date().toISOString()} [STDERR]: ${data}`;
    ...
    serverProcess.on('exit', (code) => {
      const logData = `${new Date().toISOString()} [EXIT]: Server process exited with code ${code}\n`;
      console.log(logData);
      logStream.write(logData);
      logStream.end();
      
      // Reject on ANY exit before server is ready
      reject(new Error(`Server exited with code ${code} before startup completed`));
    });

    // Use health check instead of stdout parsing
    setTimeout(async () => {
      try {
        await waitForServer();
        resolve();
    ...
    }, 2000); // Give server 2 seconds to start, then check health

    // Remove the old timeout since we handle it in the health check now
  });
}
